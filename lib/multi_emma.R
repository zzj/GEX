...
emma.REML.multi <- function(ys, idx, xs, K, Z=NULL, X0 = NULL, ngrids=100, llim=-10, ulim=10, esp=1e-10, ponly = FALSE) {
  if ( is.null(dim(ys)) || ncol(ys) == 1 ) {
    ys <- matrix(ys,1,length(ys))
  }
  if ( is.null(dim(xs)) || ncol(xs) == 1 ) {
  ...
  if ( sum(is.na(ys)) == 0 ) {
    ...
    for(i in 1:size) {
      end1 <- start1+idx[i]-1
      vids <- !is.na(xs[start1,])
      nv <- sum(vids)
      xv <- xs[start1:end1,vids]
      if ( ( mean(xv) <= 0 ) || ( mean(xv) >= 1 ) ) {
        if ( !ponly ) {
          vgs[i,] <- rep(NA,g)
          ves[i,] <- rep(NA,g)
        ...
        if ( is.null(Z) ) {
          if (end1-start1==0)
            X <- cbind(X0[vids,,drop=FALSE],(xs[start1:end1,vids]))
          else
            X <- cbind(X0[vids,,drop=FALSE],t(xs[start1:end1,vids]))
            
          eig.R1 = emma.eigen.R.wo.Z(K[vids,vids],X)
        }
        else {
          vrows <- as.logical(rowSums(Z[,vids]))              
          X <- cbind(X0[vrows,,drop=FALSE],Z[vrows,vids,drop=FALSE]%*%t(xs[start1:end1,vids,drop=FALSE]))
          eig.R1 = emma.eigen.R.w.Z(Z[vrows,vids],K[vids,vids],X)
        }
#        print(X[1:10,])
        for(j in 1:g) {
          if (nv == t){
            ...
            for (w1 in 1:(idx[i])){
              for (w2 in 1:(idx[i])){
                down <- down+beta[q1+w1-1]*XX[w1,w2]*beta[q1+w2-1]/REMLE$vg
              }
            }
            print(beta)
            stats[i,j] <- (down)/(q1-1)
            if (stats[i,j]<0) stats[i,j]=0;
          }
          else {
            if ( is.null(Z) ) {
              eig.L0 <- emma.eigen.L.wo.Z(K[vids,vids])
        ...
        ps[i,] <- pf(abs(stats[i,]),q1-1,dfs[i,],lower.tail=FALSE)
      }
      start1 <- end1+1
    }
  }
...
emma.REML.double <- function(ys, xs, K, Z=NULL, X0 = NULL, ngrids=100, llim=-10, ulim=10, esp=1e-10, ponly = FALSE) {
  if ( is.null(dim(ys)) || ncol(ys) == 1 ) {
    ys <- matrix(ys,1,length(ys))
  }
  if ( is.null(dim(xs)) || ncol(xs) == 1 ) {
  ...
  if ( sum(is.na(ys)) == 0 ) {
    ...
    for(i in 2) {
      ...
      if ( ( mean(xv) <= 0 ) || ( mean(xv) >= 1 ) ) {
        ...
        if ( is.null(Z) ) {
          X <- cbind(X0[vids,,drop=FALSE],xs[i:(i+1),vids])
          eig.R1 = emma.eigen.R.wo.Z(K[vids,vids],X)
        }
        else {
          vrows <- as.logical(rowSums(Z[,vids]))              
          X <- cbind(X0[vrows,,drop=FALSE],Z[vrows,vids,drop=FALSE]%*%t(xs[i:(i+1),vids,drop=FALSE]))
          eig.R1 = emma.eigen.R.w.Z(Z[vrows,vids],K[vids,vids],X)
        }
